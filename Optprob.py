...
class Groundstructure:
    ...
    def premembers(self, preml):
        for ml in self.ML:
            for pl in preml:
                if ((ml.start.position[0]==pl.start.position[0] and ml.start.position[1]==pl.start.position[1]) and (ml.end.position[0]==pl.end.position[0] and ml.end.position[1]==pl.end.position[1])):ml.area = 1.0
            
class Optprob:
    def __init__(self, domain, supports, loads, material, preml, step1, step2):
    ...
    def initial(self):
        ...
        self.groundstructure.premembers(self.preml)
    def MatrixB(self):
